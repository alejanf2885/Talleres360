    ...
    public class UsuarioService : IUsuarioService
    {
            ...
            int tallerId, string nombre, string email, string password, string? rutaImagen = null)
        {
            ...
            try
            {
                ...
                {
                    _logger.LogWarning(
                        "Post-save check: no se encontr� UsuarioId={UsuarioId} en BD (sin filtros).",
                        usuario.Id);
                }
                else
                {
            ...
            {
                _logger.LogError(ex,
                    "Error cr�tico creando usuario admin para taller {TallerId}",
                    tallerId);

                return ServiceResult<Usuario>.Fail(
                    ErrorCode.SYS_ERROR_GENERICO.ToString(),
                    "Fallo cr�tico en la base de datos.");
            }
        }
